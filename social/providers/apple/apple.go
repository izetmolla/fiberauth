...
func (Provider) UnmarshalSession(data string) (social.Session, error) {
	s := &Session{}
...
func (p Provider) FetchUser(session social.Session) (social.User, error) {
	s := session.(*Session)
	if s.AccessToken == "" {
		return social.User{}, fmt.Errorf("no access token obtained for session with provider %s", p.Name())
	}
	return social.User{
		Provider:     p.Name(),
		UserID:       s.ID.Sub,
...
func (p Provider) RefreshToken(refreshToken string) (*oauth2.Token, error) {
	...
	ts := p.config.TokenSource(social.ContextForClient(p.Client()), token)
